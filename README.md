...
local SQLite file, works out who your merchants actually are, and serves the
result three ways: a password-protected web UI, a spending-analytics page,
and a read-only [MCP](https://modelcontextprotocol.io) server so agents
running on this machine can answer questions about your money.
...
- **Nothing here can move money.** The sync is read-only at the API layer
  ...
  read-only at the SQLite layer — a write raises no matter who asks. The
  only thing the UI can change is labels: merchant names, categories, and
  themes.
...
- **It works without an AI key.** Every deterministic feature — sync,
  ...
  model configured. An Anthropic key adds merchant identification, and its
...
- Python 3.12+
- A [Redbark](https://docs.redbark.com) API key (`rbk_live_…`) — Redbark is an
  Australian CDR open-banking aggregator; your bank consents live in their
  dashboard, and Spendglass only ever reads through them. Data is
  AUD-flavoured and CDR-shaped (16 fixed bank categories, 12-month
  consents).
...
to set a durable password; after that you just log in. Set
...
model only reads answers. The tool list is pinned by a test: adding a tool
is a reviewed decision, because the tool surface is the security surface.
Every response carries `as_of` and `stale` flags — a stale store answers
...
(cheap); everything else is deterministic SQL — free. Discovery is
front-loaded: once coverage is built, only new merchants trigger lookups.
...
Loopback-only binds with a Host-header allowlist (DNS-rebinding defense),
